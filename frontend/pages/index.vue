<template>
  <div class="container">
    ...
    <div v-if="isAuth">
      <section>
        <b-button @click="onGraphQL" type="is-primary my-4">
          Save item using GraphQL AppSync
        </b-button>
        <div>
          <b-field class="my-4">
            <input v-model="note.id" type="text" name="id">
          </b-field>
          <b-field class="my-4">
            <input v-model="note.content" type="text" name="content">
          </b-field>
        </div>
      </section>
      <span>Subscribed for test 'hanna-banana' only, show content: {{ subscriptionText }}</span>
      <br/>
      <span>All notes: {{ listNotes }}</span>
      <hr>
      <div>
...
import { CreateNoteGql, NoteInput, CreateNoteMutation, ListNotesGql, OnCreateNoteSubscriptionGql } from '../generated'
...
export default class IndexPage extends Vue {
  ...
  async mounted () {
    ...
    if (isLogin) {
      const sub = await API.graphql(graphqlOperation(OnCreateNoteSubscriptionGql, { content: 'hanna-banana' })) as Observable<object>
      sub.subscribe({
        next: (data: any) => {
          this.subscriptionText = data.value.data
        },
        error: (error: any) => console.log('ws data error', error)
      })
    }
  }
  ...
  async onGraphQL () {
    ...
    this.listNotes = []
    await API.graphql(graphqlOperation(CreateNoteGql, { note: this.note })) as GraphQLResult<CreateNoteMutation>
    this.listNotes = await API.graphql(graphqlOperation(ListNotesGql, {}))
  }
